...
import {
  NoFreeDeviceError, deviceLabel, matchesDeviceFilter, parseWdaUrl, registerAdbKey, releaseDevice,
...
function fakeSdk(calls, { failOn } = {}) {
  class Agent {
    constructor(device, options) { this.device = device; calls.push(["agent", device.target, options.reportFileName]); }
...
test("Midscene Android ajanı Farm cihazında adımları yürütür", async () => {
  const calls = [];
  const outcome = await runMobileCases({
    ...
    model: { config: {} }, reportDir: mkdtempSync(join(tmpdir(), "mtr-m-")), sdk: fakeSdk(calls),
  });
  assert.equal(outcome.results[0].status, "passed");
  assert.deepEqual(calls.slice(0, 4), [["device", "10.0.0.5:7401", "/x/adb"], ["agent", "10.0.0.5:7401", "run-5-case-7"], ["terminate", "com.demo"], ["launch", "com.demo"]]);
