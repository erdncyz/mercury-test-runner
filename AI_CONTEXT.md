...
- `src/midscene.mjs`: model ayarını Midscene ajan başına `modelConfig`'e çevirir, model ailesini algılar; `executeCases` ortak adım motorudur (adım başına zaman aşımı `stepTimeoutMs`, güvenli tek yeniden deneme `shouldRetry`, düşen adımda engel kurtarma `canRecover`/`recoverFromInterruption` (AI çerez/pop-up'ı kapatır, yutulan önceki eylemi yeniden yapar, adımı bir kez daha dener; `recover: false`, `MERCURY_STEP_RECOVERY=0`), case başına `{{saved.*}}` değerleri, `item.context` → `aiContexts.default`, `aiContexts.aiAct` = `INTERRUPTION_HINT` + bağlam). `runStep` adım türlerini Midscene API'lerine eşler; yeni adım türü eklerken `stepLabel`, `parseSteps` (`INLINE_ARG`), `qa-agent.mjs` (`ACTION_ALIASES`/`normalizeStep`/istem) ve README tablosu birlikte güncellenir. `runWebCases` Playwright + `@midscene/web/playwright/agent` (bağlam `realUserAgent` ile "HeadlessChrome"suz kimlik alır; WAF engelini önler), `runMobileCases` `@midscene/android` (`AndroidDevice`/`AndroidAgent`) ve `@midscene/ios` (`IOSDevice`/`IOSAgent`) kullanır
